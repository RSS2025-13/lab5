...
class ParticleFilter(Node):
    ...
    def timer_callback(self):
        if self.initiated:
            ...
            probs = self.sensor_model.evaluate(new_particles, self.scans)
            ...
            self.particles = new_particles[np.random.choice(np.shape(new_particles)[0], size=np.shape(new_particles)[0], p=probs.flatten())]
    ...
    def pose_callback(self, msg):
        ...
        x = np.random.normal(loc=x, scale=1.0, size=(100,1))
        y = np.random.normal(loc=y, scale=1.0, size=(100,1))
        theta = np.random.normal(loc=angle, scale=1.0, size=(100,1))
